#pragma once
// 昇腾 NPU(CANN/AscendCL)后端内部头(不进 include/;仅供 src/backends/ascend/ 使用)。
...
// 昇腾 NPU 设备内存分配器(骨架)。
class AscendAllocator final : public hal::Allocator {
 public:
  Result<void*> allocate(size_t bytes, size_t alignment) override;
  void deallocate(void* ptr) override;
};

// 昇腾 NPU 后端:实现 Backend HAL 全部接口(骨架)。
class AscendBackend final : public hal::Backend {
 public:
