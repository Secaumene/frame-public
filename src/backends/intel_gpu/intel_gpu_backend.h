#pragma once
// Intel GPU(oneAPI/SYCL)后端内部头(不进 include/;仅供 src/backends/intel_gpu/ 使用)。
...
// Intel GPU 设备内存分配器(骨架)。
class IntelGpuAllocator final : public hal::Allocator {
 public:
  Result<void*> allocate(size_t bytes, size_t alignment) override;
  void deallocate(void* ptr) override;
};

// Intel GPU 后端:实现 Backend HAL 全部接口(骨架)。
class IntelGpuBackend final : public hal::Backend {
 public:
